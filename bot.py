...
logging.basicConfig(
    ...
    level=logging.ERROR if int(os.environ.get("ENV_ACTION")) else logging.INFO,
    datefmt="%Y-%m-%d %H:%M:%S",
)

BOT_TOKEN = os.environ.get("TOKEN")
DEV_IDS = [int(s.strip()) for s in os.environ.get("DEVS").split(",")]
...
airtable = AirTable(
    app_id=os.environ.get("AIRTABLE_APP"),
    table_id=os.environ.get("AIRTABLE_TABLE_PROJECT"),
    table_country_id=os.environ.get("AIRTABLE_TABLE_COUNTRY"),
    table_skill_id=os.environ.get("AIRTABLE_TABLE_KEYWORD"),
...
def delete_rss(update: telegram.Update, context: CallbackContext):
    ...
    context.bot.send_message(
        chat_id=update.effective_chat.id, text=f"Deleted {rss_name} RSS"
    )
...
def resume_updates_cb(update: telegram.Update, context: CallbackContext):
    ...
    if len(jobs) == 0:
        add_job_to_queue(
            user_id, timedelta(minutes=REPEAT_PERIOD), timedelta(minutes=REPEAT_PERIOD)
        )
    context.bot.send_message(
        chat_id=update.message.chat_id,
...
def set_settings_cb(update: telegram.Update, context: CallbackContext):
    ...
    if value not in valid_values:
        context.bot.send_message(
            chat_id=update.effective_chat.id, text=ALLOWED_SETTINGS[keyword]["error"]
        )
        return
    users_db.set_user_settings(update.message.chat_id, keyword, value)
    context.bot.send_message(
        chat_id=update.effective_chat.id, text=f"Successfully set {keyword} = {value}!"
    )
...
def clear_filter_cb(update: telegram.Update, context: CallbackContext):
    ...
    context.bot.send_message(
        chat_id=update.effective_chat.id, text=f"Successfully cleared filter {keyword}"
    )
...
def run_job_cb(update: telegram.Update, context: CallbackContext):
    ...
    try:
        ...
        context.bot.send_message(
            chat_id=update.effective_chat.id, text="Update completed"
        )
...
def id_cb(update: telegram.Update, context: CallbackContext):
    context.bot.send_message(
        chat_id=update.effective_chat.id, text=f"Your ID: {update.effective_chat.id}"
    )
